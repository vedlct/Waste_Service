    ...
    <section className="relative overflow-hidden border-y border-sky-100 bg-sky-50 text-[#102a4c]">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-28">
        <div className="grid gap-6 border-b border-white/15 pb-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-end lg:gap-16 lg:pb-14">
          <div>
            <div className="flex items-center gap-3 text-xs font-extrabold uppercase tracking-[0.19em] text-[#1294E9] sm:text-sm">
              <span className="h-0.5 w-9 bg-[#1294E9]" aria-hidden="true" />
              The service difference
            </div>
            <h2 className="mt-5 text-3xl font-bold leading-[1.04] tracking-tight sm:text-4xl lg:text-4xl">
              Less hassle at
              <span className="block font-serif font-medium italic text-[#1294E9]">every stage.</span>
            </h2>
          </div>
        ...
        <ol className="relative mt-8 space-y-4">
          {highlights.map(({ number, eyebrow, title, description, detail, icon: Icon }) => (
            <li
              key={number}
              className="group relative overflow-hidden rounded-2xl border border-sky-300 bg-white/75 px-4 shadow-sm ring-1 ring-sky-200/80 transition-all duration-500 hover:-translate-y-1 hover:border-[#1294E9] hover:bg-[#1294E9]/10 hover:text-[#081f3d] hover:shadow-xl hover:ring-2 hover:ring-[#1294E9]/50 sm:px-6"
            >
              <div className="grid gap-5 py-8 transition-transform duration-500 group-hover:translate-x-1 sm:grid-cols-[4rem_1fr] sm:items-start sm:gap-6 lg:grid-cols-[5rem_4.5rem_1fr_1.05fr_auto] lg:items-center lg:gap-8 lg:py-9">
                <span className="inline-flex w-fit rounded-full border border-[#1294E9]/45 bg-[#1294E9]/10 px-3 py-1.5 text-sm font-black tracking-[0.16em] text-[#1294E9] transition-colors duration-300 group-hover:text-[#081f3d]/55">
                  {number}
                </span>

                <span className="hidden size-14 place-items-center rounded-full border-2 border-sky-300 bg-sky-50 text-[#081f3d] shadow-sm ring-4 ring-white/80 transition duration-500 group-hover:rotate-6 group-hover:border-[#081f3d]/20 group-hover:bg-[#081f3d] group-hover:text-white sm:grid lg:size-16">
                ...
                <div className="flex items-center justify-between gap-4 sm:col-start-2 lg:col-start-auto lg:block lg:text-right">
                  ...
                  <ArrowUpRight className="mt-0 size-6 text-[#1294E9] transition duration-500 group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-[#081f3d] lg:ml-auto lg:mt-3" aria-hidden="true" />
                </div>
              </div>
            </li>
          ))}
        ...
        <div className="mt-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link
            href="/#prices"
            className="group inline-flex min-h-13 shrink-0 items-center justify-center gap-3 rounded-full bg-white px-6 py-3.5 text-sm font-extrabold text-[#081f3d] hover:text-white transition duration-300 hover:-translate-y-0.5 hover:bg-[#1294E9] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white sm:text-base"
          >
            Explore prices &amp; book
