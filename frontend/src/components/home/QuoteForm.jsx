...
export default function QuoteForm() {
  return (
    <section id='form' className='relative overflow-hidden bg-linear-to-br from-[#B9E8FA]/20 to-white py-16 text-[#102a4c] sm:py-20 lg:py-28'>
      ...
      <div className='relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'>
        <div className='mx-auto max-w-5xl text-center'>
          <span className='inline-flex rounded-full border border-white/20 bg-[#0492E8]/30 px-4 py-2 text-xs font-black uppercase tracking-[0.22em] shadow-sm backdrop-blur' style={{ color: '#102a4c' }}>
            Need a little guidance?
          </span>

          <h2 className='mt-7 text-3xl font-black leading-[1.02] tracking-[-0.05em] sm:text-4xl lg:text-4xl'>
            You don&apos;t need to work out
            <span className='block text-[#0492E8]'>every detail on your own.</span>
          </h2>
          ...
          <div className='mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row'>
            <Link href='/contactUs' className='group inline-flex w-full items-center justify-center gap-3 rounded-full bg-white px-6 py-4 text-sm font-black text-[#11224D] hover:text-white shadow-xl transition-all duration-300 hover:-translate-y-1 hover:bg-[#11224D] hover:shadow-2xl focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white sm:w-auto sm:text-base'>
        ...
        <div className='mt-12 grid grid-cols-1 gap-4 sm:mt-14 md:grid-cols-3'>
          {steps.map((step, index) => {
            ...
            return (
              <article key={step.title} className='group relative overflow-hidden rounded-[1.75rem] border border-white/55 bg-[#0492E8] p-5 text-left shadow-sm backdrop-blur-md transition-all duration-500 hover:-translate-y-2 hover:border-white/80 hover:bg-[#0492E8]/25 hover:shadow-2xl sm:p-6'>
                <div className='flex items-start justify-between gap-4'>
                  <span className='flex size-12 items-center justify-center rounded-2xl bg-white text-[#11224D] group-hover:text-white transition-all duration-500 group-hover:rotate-6 group-hover:scale-110 group-hover:bg-[#11224D]'>
                    <Icon aria-hidden='true' className='size-6' />
                  </span>
                  <span className='text-xs font-black tracking-[0.18em] text-white/60 group-hover:text-[#102a4c]/35'>{String(index + 1).padStart(2, '0')}</span>
                </div>
                <h3 className='mt-7 text-xl font-black text-white group-hover:text-[#102a4c] sm:text-2xl'>{step.title}</h3>
                <p className='mt-3 text-sm leading-6 text-white/60 group-hover:text-[#102a4c]/65 sm:text-base'>{step.text}</p>
                <span className='absolute inset-x-6 bottom-0 h-1 origin-left scale-x-0 rounded-full bg-[#11224D] transition-transform duration-500 group-hover:scale-x-100' />
              </article>
            )
          })}
        </div>
        ...
        <div className='mt-8 flex flex-col gap-4 border-t border-white/15 pt-6 text-sm text-[#102a4c]/65 sm:flex-row sm:items-center sm:justify-between'>
          <p className='inline-flex items-center gap-2'>
            <ShieldCheck aria-hidden='true' className='size-5 shrink-0 text-[#0492E8]' />
