      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-28">
        <div className="grid gap-8 border-b border-white/15 pb-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end lg:gap-16 lg:pb-16">
          <div>
            ...
            <h2 className="mt-5 text-3xl font-bold leading-[1.04] tracking-tight sm:text-4xl lg:text-4xl">
              From dumped waste
              <span className="block font-serif font-medium italic text-[#0492E8]">to a clear space.</span>
            </h2>
          </div>
          <div>
        ...
        <ol>
          {servicePromises.map(({ icon: Icon, title, description }, index) => (
            <li key={title} className="group grid gap-5 border-b border-white/15 py-8 transition-colors duration-500 hover:bg-[#0492E8] hover:text-[#081f3d] md:grid-cols-[4rem_4rem_0.8fr_1.2fr] sm:items-center sm:px-4 lg:gap-8 lg:py-9">
              <span className="text-sm font-black tracking-[0.15em] text-[#0492E8] transition-colors group-hover:text-white">0{index + 1}</span>
              <span className="grid size-13 place-items-center rounded-full border border-white/20 text-[#0492E8] transition duration-500 group-hover:rotate-6 group-hover:border-[#081f3d] group-hover:bg-[#081f3d] group-hover:text-white">
                <Icon className="size-6" strokeWidth={1.8} aria-hidden="true" />
              </span>
              <h3 className="text-xl font-bold sm:text-2xl group-hover:text-white">{title}</h3>
              <p className="text-sm leading-7 text-[#102a4c]/62 transition-colors group-hover:text-white sm:text-base">{description}</p>
            </li>
          ))}
        </ol>
        ...
        <div className="mt-14 grid gap-8 rounded-[2rem] border border-white/15 bg-white/5 p-6 sm:p-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-12 lg:p-10">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[0.17em] text-[#0492E8]">Waste we can assess</p>
            <h3 className="mt-4 text-3xl font-bold leading-tight sm:text-4xl">Mixed rubbish, handled by one local team.</h3>
          </div>
          <ul className="grid gap-x-8 sm:grid-cols-2">
            {wasteTypes.map((item) => (
              <li key={item} className="group/item flex min-h-12 items-center gap-3 border-b border-white/12 py-3 text-sm font-semibold text-[#102a4c]/78 transition hover:translate-x-1 hover:border-[#0492E8] hover:text-white sm:text-base">
                <Check className="size-4 shrink-0 text-[#0492E8] transition-transform group-hover/item:scale-110" strokeWidth={3} aria-hidden="true" />
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className="mt-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link href="/contactUs" className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#0492E8] px-6 py-3 text-sm font-extrabold text-white hover:text-[#081f3d] transition duration-300 hover:-translate-y-0.5 hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white">
              Request a clearance
