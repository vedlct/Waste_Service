...
export function generateMetadata() {
  return metadataFor('/garageClearance', { title: 'Garage Clearance | MR. TEE Removals' })
}
...
export default async function page () {
  const [service, contact] = await Promise.all([getService('garage-clearance'), getSiteContact()])

  return (
    <div>
        <GarageHero service={service} phone={contact.phone} />
        <RubbishService/>
        <GetPrices/>
        <HowItWorks/>
