...
export function generateMetadata() {
  return metadataFor('/warehouseClearance', { title: 'Warehouse Rubbish Clearance | MR. TEE Removals' })
}
...
export default async function page () {
  const [service, contact] = await Promise.all([getService('warehouse-clearance'), getSiteContact()])

  return (
    <div>
        <WareHouseHero service={service} phone={contact.phone} />
        <SecondaryNav/>
        <ServiceList/>
        <WhyChooseUs/>
        <JunkRubbishItem/>
